  ...
  export default {
    ...
    methods: {
      ...
      onSubmit(){
        let bf = this.bf
        let obj = bf.getData()
        obj.id = window.localStorage.recharge_merchant_id
        if(bf.validate()){
          axios.put(api.RECHARGE_MERCHANT_LIST, obj)
          .then(res => {
            console.log(res)
            this.$message(res);
          }).catch(e => {
      ...
      load(){
        let recharge_merchant_spec = JSON.parse(window.localStorage.recharge_merchant_obj)
        console.log(recharge_merchant_spec)
        ...
        this.bf.render(container,recharge_merchant_spec);
      }
    },
    watch:{
    }

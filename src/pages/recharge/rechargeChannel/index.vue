...
<!--       <div>
      ...
      <el-button type="primary" v-if="!!fpUi&&fpUi.add.state" icon="plus" @click="add">新增渠道</el-button>
    </h3>
     <el-table
    :data="lottory"
    border
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      add(){
        this.$router.push({ path: '/rechargeChannel/add'})
      },
      modify(row){
      ...
      load(){
        var  parms = {}
        axios.get(api.FUNDACCOUNT, parms)
          .then(res => {
            console.log(res)
            this.loading = false
          }).catch(e => {

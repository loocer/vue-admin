...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      onSubmit(){
        this.$refs['form-hrty'].validate((valid) => {
          ...
          if (valid) {
            ...
            this.form.lotteryId = window.localStorage.lottory_award_modify
            this.form.issueConfigList = dateArray
            this.form.is_enabled = ~~this.form.is_enabled
            console.log(this.form)
            axios.post(api.AWARD_MODIFY, this.form).then(res => {
                this.$message('修改成功！');
            }).catch(e => {
                console.log(e)
            })
      ...
      load(){
        let lotteryId = window.localStorage.lottory_award_modify
        axios.post(api.AWARD, {lottery_id: lotteryId})
          .then(res => {
            console.log(res)
          let ruls = res.data.rows.issueConfigList
          for(let o in ruls){
            let rul = {
              id:Date.parse(new Date()),
              sale_cycle:ruls[o].sale_cycle,
              first_issue_bonus_time_string:new Date(ruls[o].first_issue_bonus_time),
              last_issue_bonus_time_string: new Date(ruls[o].last_issue_bonus_time),
              whatDate:null
            }
            this.setTimeData.push(rul)
          }
          this.form.dateFormat = res.data.rows.issueCodeRule.dateFormat
          this.form.numberCleaning = res.data.rows.issueCodeRule.numberCleaning
          this.form.zeroPadding = res.data.rows.issueCodeRule.zeroPadding
          this.form.number = res.data.rows.issueCodeRule.number
          this.form.lottery_cnname = res.data.rows.lottery.lottery_cnname
          this.form.lottery_enname = res.data.rows.lottery.lottery_enname
          this.form.lottery_desc = res.data.rows.lottery.remark
          this.form.is_enabled = !!~~res.data.rows.lottery.is_enabled
        }).catch(e => {
            this.$message(e);
        })
      }
